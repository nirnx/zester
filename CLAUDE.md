...
| `pkg/event` | Event wire type + subject/tag conventions for the reactor. `Event{id, tag, data, ts, v, origin, depth}` (msgpack, additive-only, key set pinned by conformance test). Subjects: `zester.event.<peelID>.send.<dotted-tag>` \| `zester.event.<peelID>.beacon.<name>` \| `zester.event._master.<dotted-tag>` \| `zester.event._admin.send.<dotted-tag>`. `ParseSubject` derives origin + slash tag with adversarial validation (min 4 tokens, no empty/wildcard tokens, `_`-origins other than `_master`/`_admin` rejected) — the SUBJECT is authoritative, payload identity is untrusted. `DottedTag`/`SlashTag` are 1:1 (`ValidateTag` allows only `[a-zA-Z0-9_-]` per segment). `MatchKey(origin, tag)` = `"<origin>/<slashTag>"` — rules match origin-scoped keys, so a compromised peel can never match `_master/...` rules. |
| `pkg/reactor` | Master-side reactor engine. `rules.go`: `reactor/top.zy` parse (ordered single-key entries, bare-list or `{react, throttle}` options form; fnmatch match-key globs, `'*'` crosses `'/'`; ALL matching rules fire in file order). `loader.go`: rule loading over the `reactor-files` bucket with mandatory `_manifest` SHA-256 verify (content-without-manifest = torn publish), `_revision` debounced hot reload (2s + 5s jitter), atomic snapshot swap, last-known-good on ANY load failure, enroll-lint warnings. `render.go`: Jinja-rendered reaction files (`ModuleFn=nil` — `salt[...]` is a render error; context `event.{id,tag,peel,origin,depth,ts,data}` + aliases `tag`/`data` + `origin_facts`); actions normalized + post-render validated (function `^[a-z0-9_]+\.[a-z0-9_]+$`, target must parse, `max_targets`); one invalid block = NO actions execute. Actions: `dispatch.module`, `dispatch.state` (sls/highstate), `local.*` Salt sugar, `enroll.approve/reject/revoke` (compile-mandatory `require_peel` + executor-enforced `_master`-origin gate), `event.send` (chaining; deterministic `DeriveChainID` + JetStream MsgID dedup; refused at depth cap or when chaining disabled), `log`. `actions.go`: deterministic JIDs `rxn-<sha256(origin,eventID,rule,block)>[:32]`, `job.ErrJIDConflict` → duplicate (exactly-once side effects), provenance metadata. `engine.go`: shared durable consumer `"reactor"` on the `events` stream (DeliverNew, AckWait 60s, MaxDeliver 5, MaxAckPending 64), consume-gate order malformed→decode→spoof→depth→ratelimit→stale (all Ack+drop+metric; stale Warns loudly), worker pool (default 4) with 30s render timeout + panic recovery + InProgress keepalive, Ack-after-complete, NakWithDelay on transient failures/backpressure. `limiter.go`: per-source token bucket (120/min burst 30, LRU-bounded), per-(rule,source) throttle, per-rule storm breaker (60/min → open 5m); throttle and breaker are TWO-PHASE — checked before a rule executes (`Throttle.Allow`/`Breaker.Allow`), recorded only after it completes non-transiently (`Throttle.Record`/`Breaker.RecordFire` in `engine.process`) so a transiently failed attempt never consumes guard state and its Nak redelivery retries; `Breaker.Sweep`/`OpenRules` (surfaced as `Engine.SweepBreakers`/`OpenBreakers`) close expired cooldowns without a new event and feed the breaker-open readiness/gauge. `service.go`: `zester.reactor.test` request/reply dry-run (queue `zester-reactor-testers`). |
| `pkg/beacon` | Peel-side beacon engine. v1 ships ONE beacon — `service` — polling `exec.ServiceExec` running state and emitting transition events on `bus.BeaconSubject(peelID, "service")` (tag `beacon/<peelID>/service`, data `{service, running, previous}`). Config via the settings pipeline key `beacons:` (`{service: {services: {<name>: {}}, interval: 10, onchangeonly: true}}`; field errors reject the whole section), hot-reloaded on settings changes, snapshot warm-started. `Manager`: poll loop with hot-swappable config, busy-skip during mutating exec (`BusyFn`), 256-entry FIFO offline buffer (oldest dropped at Debug, drained in order on reconnect), `OnEvent` hook → `zester_peel_beacon_events_total{beacon}`. Full beacon set (disk/mem/load/filewatch) is Phase 2. |
...
| `pkg/auth` | NKey/Curve25519 key management, encryption, JWT grants. Peel JWTs are least-privilege (`PeelUserJWTOptions`): job ack/return/schedule publish, own facts/basket/heartbeat writes, own-event publish (`zester.event.<peelID>.>` — feeds the reactor; NO `zester.event.>` subscribe, no reactor/admin grants), settings/secrets/state-files reads, plus publish on `zester.target.resolve`; admin JWTs (`AdminUserJWTOptions`) grant `zester.admin.>`, `zester.target.resolve`, pub `zester.event._admin.>` (`zester event send`) + `zester.reactor.>` (`zester reactor test`), and sub `zester.event.>` (`zester event watch`) — pre-reactor admin creds work degraded (no event/reactor CLI) until re-issued. Heartbeat puts and basket target resolution both ride these grants: a peel whose credentials lack them looks offline in presence views (heartbeat puts fail at Debug, retried every tick) and falls back to facts-KV scans for basket resolution (degraded, not broken). |
...
| `pkg/proto` | Shared message types (ExecRequest/ExecResponse, StateResult) + wire versioning: `ProtocolVersion = 1` (`version.go`); `ExecRequest.V`/`ExecResponse.V` (msgpack `v,omitempty`) stamped by senders — decoded `V == 0` means the field was never set and MUST be treated as compatible. `ExecRequest.ReactorDepth` (msgpack `rdepth,omitempty`, additive) threads the reaction chain depth into peel-side `event.send`. The **additive-only policy** is normative in `doc.go`: never rename/retype/remove a msgpack field (retired keys stay reserved forever), new fields are `,omitempty` with safe zero values; bump `ProtocolVersion` only for semantically breaking changes and gate rollouts via `update.Manifest.MinProtocol`. Key-set conformance tests pin the exact encoded keys of the wire structs. |
...
Salt-style reactor: events publish under `zester.event.>` (msgpack `event.Event`), the `events` JetStream stream (7d, 1 GiB, 1M msgs, MsgID dedup window 2m) captures them durably, and the masters' shared durable consumer `"reactor"` (DeliverNew, AckWait 60s, MaxDeliver 5) delivers each event to exactly ONE master — no reactor leader, capacity scales with masters, downtime replays from last ack.

- **Identity is the subject token** (never the payload): peel JWTs allow publishing only `zester.event.<own-id>.>`; the payload `tag` is cross-checked against the subject-derived tag (mismatch = spoof drop); enrollment's `ValidatePeelID` bans dots/leading-`_`/wildcards so peel origins can never collide with `_master`/`_admin`. Rules match `"<origin>/<slashTag>"` keys (`event.MatchKey`), so a compromised peel can only spoof itself.
- **Rules**: `reactor/top.zy` (plain YAML, ordered, ALL matching entries fire; fnmatch globs where `'*'` crosses `'/'`; per-rule `throttle`) + Jinja-rendered reaction `.zy` files. Distributed via the master-only `reactor-files` bucket (files → `_manifest` → `_revision`; publisher-lease-gated; empty dir never wipes a populated bucket); the loader verifies SHA-256 against the manifest, hot-reloads on `_revision` (debounced), and keeps last-known-good on ANY failure (degraded `reactor` readiness + `zester_reactor_rule_errors_total`).
- **Exactly-once side effects** on at-least-once delivery: reaction JIDs are content-addressed (`rxn-<sha256(origin,eventID,rule,block)>[:32]`; `job.ErrJIDConflict` → duplicate-suppressed), derived events get deterministic IDs published with JetStream MsgID, enroll transitions dedup in the state machine. Ack only after all matched rules complete or fail permanently; transient failures NakWithDelay.
- **Loop guards** (chaining is ON by default — these are correctness-critical): depth cap 3 (`Event.Depth`; propagated through reaction jobs via `Job.Metadata["reactor_depth"]` → `ExecRequest.ReactorDepth` → peel `event.send`), per-(rule,source) throttle and per-rule storm breaker (60/min → open 5m; both TWO-PHASE: recorded only on non-transient completion, so failed-attempt redeliveries are never throttled away; expired breakers swept every 15s), per-source rate limit (120/min burst 30), staleness gate (default 1h — a longer full-master outage DROPS queued events with a loud Warn + `reason="stale"` metric; `max_event_age: 0` = full replay).
...
- **Config**: master `reactor.*` section (enabled default true, dir `/data/reactor`, workers 4, max_chain_depth 3, enable_chaining true, default_throttle 0, source_rate_limit 120, max_event_age 1h, storm_rate 60, breaker_cooldown 5m; config `0` = disabled is translated to the pkg convention negative = disabled / 0 = default by `gateInt`/`gateDuration` in `internal/masterd/reactor.go`; the duration knobs are `config.Duration` — YAML accepts time.ParseDuration strings AND bare `0`, which plain time.Duration fields reject — bound as ordinary duration flags by `bind.go`).
- **Producers**: peel `event.send` module (mutating-queue path, bare positional = tag), `pkg/beacon` service beacon (settings `beacons:` key, transition-triggered, busy-skip, 256-ring offline buffer), `zester event send` (`_admin`), master enroll-pending hook + reactor `event.send` chains (`_master`; chained events match under `_master/reaction/<tag>`).
