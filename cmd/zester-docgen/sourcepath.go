...
// established per-module file-naming convention (dots become underscores;
// e.g. "pkg.removed" -> pkg/state/modules/pkg_removed.go). Exec and dispatch
// kinds are not migrated yet (Phase 1+); their convention is recorded here so
// the anatomy renderer has one source of truth once they are.
func sourcePath(kind modschema.Kind, module string) string {
	file := strings.ReplaceAll(module, ".", "_") + ".go"
	switch kind {
	case modschema.KindState:
		return "pkg/state/modules/" + file
