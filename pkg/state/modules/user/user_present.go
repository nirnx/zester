...
var userPresentSpec = regdef.MustSpec("user.present", modschema.KindState, UserPresent{}, modschema.Doc{
...
func (u *UserPresent) Check(ctx context.Context) (state.CheckResult, error) {
	...
	if len(u.Groups) > 0 {
		...
		if !famshared.StringSliceEqual(info.Groups, desiredGroups) {
...
func (u *UserPresent) Apply(ctx context.Context) (state.ApplyResult, error) {
	...
	if len(u.Groups) > 0 {
		...
		if !famshared.StringSliceEqual(info.Groups, desired) {
			opts.Groups = &desired
			changed = true
		}
	}
...
func (u *UserPresent) Revert(ctx context.Context) (state.ApplyResult, error) {
	...
	if u.original != nil {
		...
		if !famshared.StringSliceEqual(info.Groups, u.original.Groups) {
...
func (u *UserPresent) desiredGroups(current *exec.UserInfo) []string {
	...
	for _, og := range u.OptionalGroups {
		if famshared.ContainsString(current.Groups, og) && !famshared.ContainsString(desired, og) {
	...
	if !u.RemoveGroups {
		for _, g := range current.Groups {
			if !famshared.ContainsString(desired, g) {
