...
func (h *HostPresent) Check(ctx context.Context) (state.CheckResult, error) {
	current, _, err := famshared.ReadManagedFile(ctx, h.file, h.Path)
	if err != nil {
...
func (h *HostPresent) Apply(ctx context.Context) (state.ApplyResult, error) {
	current, existed, err := famshared.ReadManagedFile(ctx, h.file, h.Path)
	if err != nil {
...
func (h *HostPresent) Revert(ctx context.Context) (state.ApplyResult, error) {
	return famshared.RevertHostsFile(ctx, h.file, h.Path, h.backup, h.backupSet, h.created, 0644, "host.present")
}
...
func renderHostPresent(content, ip, hostname string) string {
	lines := famshared.SplitHostLines(content)
	found := false
	...
	for _, line := range lines {
		...
		if lineIP == ip {
			if famshared.ContainsString(names, hostname) {
				found = true
				out = append(out, line)
			} else {
