...
var groupPresentSpec = regdef.MustSpec("group.present", modschema.KindState, GroupPresent{}, modschema.Doc{
...
func (g *GroupPresent) Check(ctx context.Context) (state.CheckResult, error) {
	...
	if len(g.Members) > 0 && !famshared.StringSliceEqual(info.Members, g.Members) {
		diffs = append(diffs, fmt.Sprintf("members %v != %v", info.Members, g.Members))
	}

	for _, u := range g.AddUsers {
		if !famshared.ContainsString(info.Members, u) {
...
func (g *GroupPresent) Apply(ctx context.Context) (state.ApplyResult, error) {
	...
	if info == nil {
		...
		if createErr != nil {
			...
			if len(g.Members) > 0 || len(g.AddUsers) > 0 {
				...
				for _, u := range g.AddUsers {
					if !famshared.ContainsString(addList, u) {
	...
	if len(g.Members) > 0 {
		...
		for _, m := range g.Members {
			if !famshared.ContainsString(info.Members, m) {
				opts.AddMembers = append(opts.AddMembers, m)
				changed = true
			}
		}
		for _, m := range info.Members {
			if !famshared.ContainsString(g.Members, m) {
	...
	for _, u := range g.AddUsers {
		if !famshared.ContainsString(info.Members, u) && !famshared.ContainsString(opts.AddMembers, u) {
			opts.AddMembers = append(opts.AddMembers, u)
			changed = true
		}
	}
	for _, u := range g.DelUsers {
		if famshared.ContainsString(info.Members, u) && !famshared.ContainsString(opts.DelMembers, u) {
...
func (g *GroupPresent) Revert(ctx context.Context) (state.ApplyResult, error) {
	...
	if g.original != nil {
		...
		for _, m := range info.Members {
			if !famshared.ContainsString(g.original.Members, m) {
				opts.DelMembers = append(opts.DelMembers, m)
				changed = true
			}
		}
