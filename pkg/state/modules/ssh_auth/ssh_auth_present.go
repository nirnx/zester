...
var sshAuthPresentSpec = regdef.MustSpec("ssh_auth.present", modschema.KindState, SSHAuthPresent{}, modschema.Doc{
...
func (s *SSHAuthPresent) Check(ctx context.Context) (state.CheckResult, error) {
	...
	current, _, err := famshared.ReadManagedFile(ctx, s.file, path)
	if err != nil {
...
func (s *SSHAuthPresent) Apply(ctx context.Context) (state.ApplyResult, error) {
	...
	current, existed, err := famshared.ReadManagedFile(ctx, s.file, path)
	if err != nil {
...
func (s *SSHAuthPresent) Revert(ctx context.Context) (state.ApplyResult, error) {
	...
	return famshared.RevertHostsFile(ctx, s.file, path, s.backup, s.backupSet, s.created, 0600, "ssh_auth.present")
}
...
func renderSSHAuthPresent(content, keyBlob, line string) string {
	existing := famshared.SplitHostLines(content)
	...
	return famshared.JoinHostLines(out)
}
