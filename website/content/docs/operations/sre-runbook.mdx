...
/etc/zester/beacons.d/          # beacon configs
...
Prometheus metrics and health endpoints are planned but not yet implemented. The metric names below describe the target design. Currently, use NATS server monitoring (`http://<nats-host>:8222/`) for observability.
...
Health check endpoints are planned but not yet implemented. The design below describes the target behavior.
