...
`zester ca` handles the TLS plane; `zester auth init` handles the NATS **auth** plane — the operator/account/user JWTs your daemons authenticate with. It is also offline (no NATS needed):
