...
| `reactor` | Only registered when the reactor is enabled: `down` while the shared durable `reactor` consumer is not running (a boot failure is retried every 60s), `degraded` while the engine runs on last-known-good rules after a failed rule load or while a rule's storm circuit breaker is open (the message lists the affected rules), `ok` otherwise — see [Reactor operations](/docs/operations/reactor) |
