...
|---------|-------------|----|------------------|
| `nats-server` | `nats-server.crt`, `nats-server.key` | `nats` | At least one `--dns` or `--ip` **required** — SANs must cover **every** name or IP peels dial (`nats_url` / `nats_advertise_urls`, plus `localhost` for a colocated watchdog) |
