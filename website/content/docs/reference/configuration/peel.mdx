...
| `startup_states` | string | `""` | Run states ONCE per process start (Salt `startup_states` parity): `highstate` applies the full highstate, `sls` applies the `startup_sls_list` refs in order, empty disables. Runs on the serialized exec path; online it waits (bounded, 60s) for the peel's first state-file sync so it compiles against current published truth (offline it proceeds with the local tree after the grace), and it retries with backoff while the peel is still coming up — a freshly provisioned peel converges autonomously (enroll → sync → apply). An invalid value aborts startup. For *recurring* enforcement use a `schedule` entry with `run_on_start: true` instead. |
| `startup_sls_list` | list | `[]` | State refs applied in order at startup when `startup_states: sls` (Salt `sls_list`). Each ref is a separate sequential run (cross-ref requisites need `include:`) and must exist in the tree the run compiles against — a missing ref reports its compile error once; only infrastructure-not-ready conditions retry. Setting the list without `startup_states: sls` aborts startup. |
...
| `--startup-states` | `""` | Run states once at startup: `highstate` or `sls` (with `--startup-sls-list`) |
| `--startup-sls-list` | `""` | Comma-separated state refs applied at startup when `--startup-states sls` |
