...
|-----------|------------------|
| `publisher` | Settings-files publish, state-files publish, and the GitFS sync loop |
...
7. Initialize the state-files publisher and construct the GitFS syncer if `gitfs.remotes` is configured.
8. Start the `publisher` leader lease — the holder publishes the settings files and state files to KV and runs the GitFS sync loop (see [Multi-Master Coordination](#multi-master-coordination)).
...
23. Start the connected-peels gauge (recounts the `peel-heartbeat` bucket every 15s into `zester_master_connected_peels`).
