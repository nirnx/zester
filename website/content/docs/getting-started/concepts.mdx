...
zester.event.<peel-id>           # Peel -> Master events
...
zester.reactor                   # Event-driven reactions (planned)
```
...
## Beacons and Reactor (Planned)

<Callout type="info" title="Not yet implemented">
Beacons and Reactor are planned features. The package stubs exist (`pkg/beacon/`, `pkg/reactor/`) but contain no implementation yet.
</Callout>

**Beacons** will monitor local conditions on peels (disk usage, file changes, etc.) and emit events to `zester.event.<peel-id>.beacon.<name>`.

**Reactor** will watch for events on the master and trigger automated responses. Events will be persisted in JetStream for replay after master restart.
...
| **Beacons** | Beacons | Peel-side monitoring (planned) | Events via JetStream |
| **Reactor** | Reactor | Event-driven automation (planned) | JetStream streams |
