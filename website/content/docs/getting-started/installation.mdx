...
```bash
# 1. Trust the repository signing key. The published key is ASCII-armored, so
#    dearmor it into the binary keyring apt expects — writing the armored bytes
#    straight to a .gpg keyring makes `apt update` fail with NO_PUBKEY.
curl -fsSL https://repo.zester.cc/debian/repository.key \
  | sudo gpg --dearmor -o /usr/share/keyrings/zester.gpg
