...
It only changes how long the watcher *waits*: a grace period (5s) after the
ack-window re-publish, targets that were heartbeat-absent at dispatch AND
never acked AND never returned are finalized with a synthetic per-peel return
