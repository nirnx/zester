...
| `zester.event.<peel-id>` | Peel -> Master | Peel events (general) | `zester.event.web-01` |
| `zester.event.<peel-id>.beacon.<name>` | Peel -> Master | Named beacon data | `zester.event.web-01.beacon.disk` |
| `zester.event.>` | Master (subscribe) | All peel events wildcard | Event aggregation |
...
| `zester.reactor` | Internal | Event-driven reactions | Reactor engine |
...
sequenceDiagram
    ...
    Note over KV: facts, settings-files, secrets,<br/>basket, jobs, job-returns,<br/>master-heartbeat, peel-heartbeat,<br/>leases, enrollments,<br/>enroll-challenges, state-files, update-*

    JS->>KV: CreateDefaultStreams()
    Note over KV: job-events stream
