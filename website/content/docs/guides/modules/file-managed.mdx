...
| `source` | `string` | No | *(none)* | local file path to copy content from; provide either content or source (source wins if both are set) |
...
| `mode` | `FileMode` | No | `0644` (lazy) | file permission mode in octal ("0644", "0755", "4755"); setuid/setgid/sticky bits honored; defaults to 0644 |
| `user` | `string` | No | *(none)* | file owner username; ownership is left unchanged when unset |
| `group` | `string` | No | *(none)* | file group name; ownership is left unchanged when unset |
| `makedirs` | `bool` | No | *(none)* | create missing parent directories (with mode 0755); a boolean that also accepts the integers 1 (true) and 0 (false) |
