...
Fails first when the target's parent directory is missing and `makedirs` is unset (the canonical file.* contract). Stats the path. An existing file (any content) needs no change; a missing path needs a change.
