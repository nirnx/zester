...
| `makedirs` | `bool` | No | *(none)* | create missing parent directories (mode 0755) before touching; a boolean that also accepts the integers 1 (true) and 0 (false) |
...
Stats the path. An existing file (any content) needs no change; a missing path needs a change.
