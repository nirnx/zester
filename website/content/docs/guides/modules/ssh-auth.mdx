---
title: "ssh_auth"
description: "The ssh_auth.* family of state modules."
---
...
| Module | Summary |
|---|---|
| [`ssh_auth.absent`](#ssh-auth-absent) | Ensure an SSH public key is absent from a user's authorized_keys. |
| [`ssh_auth.present`](#ssh-auth-present) | Ensure an SSH public key is present in a user's authorized_keys. |
...
## `ssh_auth.absent` [#ssh-auth-absent]
...
`ssh_auth.absent` also accepts the family parameters `user`, `config` — see [Family Parameters](#family-parameters).
...
## `ssh_auth.present` [#ssh-auth-present]
...
`ssh_auth.present` also accepts the family parameters `user`, `config` — see [Family Parameters](#family-parameters).
...
- [ssh_auth.absent](/docs/guides/modules/ssh-auth#ssh-auth-absent)
